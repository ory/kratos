...
func TestWebHooks(t *testing.T) {
	...
	bodyWithFlowAndIdentityAndSessionAndClaimsAndTransientPayload := func(req *http.Request, f flow.Flow, s *session.Session, c *claims.Claims, tp json.RawMessage) string {
		...
		return fmt.Sprintf(`{
					...
					"nickname": "%s",
					"groups": ["%s", "%s"]
				}`, f.GetID(), s.Identity.ID, s.ID, string(h), req.Method, "http://www.ory.sh/some_end_point", string(tp), oidcClaims.Nickname, oidcClaims.RawClaims["groups"].([]string)[0], oidcClaims.RawClaims["groups"].([]string)[1])
	}
