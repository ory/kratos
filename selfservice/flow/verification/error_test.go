...
func TestHandleError(t *testing.T) {
	...
	t.Run("flow=browser", func(t *testing.T) {
		expectVerificationUI := func(t *testing.T) (*kratos.VerificationFlow, *http.Response) {
			res, err := ts.Client().Get(ts.URL + "/error")
			require.NoError(t, err)
			defer res.Body.Close()
			assert.Contains(t, res.Request.URL.String(), conf.SelfServiceFlowVerificationUI().String()+"?flow=")

			lf, _, err := sdk.PublicApi.GetSelfServiceVerificationFlow(context.Background()).Id(res.Request.URL.Query().Get("flow")).Execute()
			require.NoError(t, err)
			return lf, res
		}
