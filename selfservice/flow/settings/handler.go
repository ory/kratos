...
func (h *Handler) RegisterPublicRoutes(public *x.RouterPublic) {
	...
	public.GET(RouteInitBrowserFlow, h.d.SessionHandler().IsAuthenticated(h.initBrowserFlow, func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		if x.IsJSONRequest(r) {
			h.d.Writer().WriteError(w, r, errors.WithStack(herodot.ErrForbidden.WithID(text.ErrNoActiveSession).WithReason("Please include a valid session cookie or session token when calling this endpoint.")))
		} else {
