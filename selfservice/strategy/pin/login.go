...
func (s *Strategy) Login(w http.ResponseWriter, r *http.Request, f *login.Flow, sess *session.Session) (i *identity.Identity, err error) {
	...
	i, err = s.d.PrivilegedIdentityPool().GetIdentityConfidential(r.Context(), sess.IdentityID)
	if err != nil {
		return nil, err
	}
...
func (s *Strategy) CompletedAuthenticationMethod(_ context.Context, _ sqlxx.JSONRawMessage) session.AuthenticationMethod {
...
func (s *Strategy) handleLoginError(_ http.ResponseWriter, r *http.Request, f *login.Flow, _ *submitSelfServiceLoginFlowWithPinMethodBody, err error) error {
	if f != nil {
