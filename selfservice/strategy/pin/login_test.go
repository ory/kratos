...
func TestCompleteLogin(t *testing.T) {
	...
	t.Run("should return an error because no pin is set", func(t *testing.T) {
		...
		var check = func(t *testing.T, body string) {
			...
			assert.Len(t, gjson.Get(body, "ui.nodes").Array(), 2)
		...
		t.Run("type=browser", func(t *testing.T) {
			hc := testhelpers.NewHTTPClientWithIdentitySessionCookie(t, ctx, reg, i)
			check(t, expectValidationError(t, false, true, hc, values))
		})

		t.Run("type=api", func(t *testing.T) {
			hc := testhelpers.NewHTTPClientWithIdentitySessionToken(t, ctx, reg, i)
