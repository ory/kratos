...
func (s *Strategy) processLogin(w http.ResponseWriter, r *http.Request, a *login.Flow, provider Provider, c *identity.Credentials, i *identity.Identity, claims *Claims) (*registration.Flow, error) {
	if err := s.updateIdentityTraits(w, r, i, provider, claims); err != nil {
		return nil, s.handleError(w, r, a, provider.Config().ID, nil, err)
	}

	var o identity.CredentialsSAML
	if err := json.NewDecoder(bytes.NewBuffer(c.Config)).Decode(&o); err != nil {
		return nil, s.handleError(w, r, a, provider.Config().ID, nil, errors.WithStack(herodot.ErrBadRequest.WithReason("The password credentials could not be decoded properly").WithDebug(err.Error())))
	}
...
func (s *Strategy) Login(w http.ResponseWriter, r *http.Request, f *login.Flow, identityID uuid.UUID) (i *identity.Identity, err error) {
	...
	goTo := urlx.AppendPaths(s.d.Config().SelfPublicURL(r.Context()), RouteBaseAuth, pid).String())
	if x.IsJSONRequest(r) {
		s.d.Writer().WriteError(w, r, flow.NewBrowserLocationChangeRequiredError(goTo))
	} else {
