...
func (s *Strategy) setTraits(w http.ResponseWriter, r *http.Request, claims *Claims, provider Provider, jsonClaims bytes.Buffer, i *identity.Identity) (err error) {
	newTraits := jsonClaims.Bytes()
	for _, remove := range []string{"iss", "email_verified", "sub"} {

		newTraits, err = sjson.DeleteBytes(newTraits, remove)
		if err != nil {
	...
	newTraits, err = sjson.DeleteBytes(newTraits, "email_verified")
	if err != nil {
		return errors.WithStack(err)
	}

	newTraits, err = sjson.DeleteBytes(newTraits, "sub")
	if err != nil {
		return errors.WithStack(err)
	}
