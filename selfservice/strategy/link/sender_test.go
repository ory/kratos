...
func TestManager(t *testing.T) {
	...
	t.Run("method=SendRecoveryLink", func(t *testing.T) {
		...
		assert.Contains(t, messages[0].Body, urlx.AppendPaths(conf.SelfPublicURL(), recovery.RouteSubmitFlow).String()+"?")
		...
		assert.NotContains(t, messages[1].Body, urlx.AppendPaths(conf.SelfPublicURL(), recovery.RouteSubmitFlow).String()+"?")
	...
	t.Run("method=SendVerificationLink", func(t *testing.T) {
		...
		assert.Contains(t, messages[0].Body, urlx.AppendPaths(conf.SelfPublicURL(), verification.RouteSubmitFlow).String()+"?")
		assert.Contains(t, messages[0].Body, "token=")
		assert.Contains(t, messages[0].Body, "flow=")

		assert.EqualValues(t, "not-tracked@ory.sh", messages[1].Recipient)
		assert.Contains(t, messages[1].Subject, "tried to verify")
		assert.NotContains(t, messages[1].Body, urlx.AppendPaths(conf.SelfPublicURL(), verification.RouteSubmitFlow).String()+"?")
