...
func (s *Strategy) PopulateVerificationMethod(r *http.Request, req *verification.Flow) error {
	f := container.New(req.AppendTo(urlx.AppendPaths(s.d.Config(r.Context()).SelfPublicURL(), RouteVerification)).String())
...
func (s *Strategy) verificationHandleFormSubmission(w http.ResponseWriter, r *http.Request, f *verification.Flow) {
	...
	if err := flow.EnsureCSRF(r, f.Type, s.d.Config(r.Context()).DisableAPIFlowEnforcement(), s.d.GenerateCSRFToken, body.Body.CSRFToken); err != nil {
		s.handleVerificationError(w, r, f, body, err)
		return
	}
