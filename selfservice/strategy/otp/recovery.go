...
func (s *Strategy) PopulateRecoveryMethod(r *http.Request, f *recovery.Flow) error {
	if s.d.Config().SelfServiceStrategy(r.Context(), s.RecoveryStrategyID()).Enabled {
		return nil
	}
...
func (s *Strategy) Recover(w http.ResponseWriter, r *http.Request, f *recovery.Flow) (err error) {
	...
	if body.Token != "" {
		...
		return s.recoveryUseToken(w, r, f, body)
	}
...
func (s *Strategy) recoveryUseToken(w http.ResponseWriter, r *http.Request, rf *recovery.Flow, body *payloadBody) error {
	tkn, err := s.d.RecoveryTokenPersister().UseRecoveryToken(r.Context(), rf.ID, body.Token)
	if err != nil {
...
func (s *Strategy) recoveryIssueSession(w http.ResponseWriter, r *http.Request, f *recovery.Flow, id *identity.Identity) error {
	...
	sess, err := session.NewActiveSession(r.Context(), id, s.d.Config(), time.Now().UTC(),
...
func (s *Strategy) retryRecoveryFlowWithMessage(w http.ResponseWriter, r *http.Request, ft flow.Type, message *text.Message) error {
	...
	f, err := recovery.NewFlow(s.d.Config(), s.d.Config().SelfServiceFlowRecoveryRequestLifespan(r.Context()), s.d.CSRFHandler().RegenerateToken(w, r), r, s.d.RecoveryStrategies(r.Context()), ft)
	if err != nil {
		return err
	}
...
func (s *Strategy) retryRecoveryFlowWithError(w http.ResponseWriter, r *http.Request, ft flow.Type, recErr error) error {
	...
	f, err := recovery.NewFlow(s.d.Config(), s.d.Config().SelfServiceFlowRecoveryRequestLifespan(r.Context()),
