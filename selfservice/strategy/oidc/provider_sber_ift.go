...
var _ OAuth2Provider = (*ProviderSberIft)(nil)

type ProviderSberIft struct {
...
func NewProviderSberIft(config *Configuration, reg Dependencies) Provider {
	if config.PKCE == "" {
		config.PKCE = "auto"
	}

	authURL := "https://id-ift.sber.ru/CSAFront/oidc/sberbank_id/authorize.do"
	if config.AuthURL != "" {
		authURL = config.AuthURL
	}

	tokenURL := "https://oauth-ift.sber.ru/ru/prod/tokens/v2/oidc"
	if config.TokenURL != "" {
		tokenURL = config.TokenURL
	}

	return &ProviderSberIft{
		config:      config,
		reg:         reg,
		authURL:     authURL,
		tokenURL:    tokenURL,
...
func (g *ProviderSberIft) Config() *Configuration {
	return g.config
}

func (g *ProviderSberIft) AuthCodeURLOptions(r ider) []oauth2.AuthCodeOption {
	return []oauth2.AuthCodeOption{
		oauth2.SetAuthURLParam("nonce", sberNonceFromRequest(g.config.ID, r)),
	}
}

func (g *ProviderSberIft) OAuth2(ctx context.Context) (*oauth2.Config, error) {
...
func (g *ProviderSberIft) Claims(ctx context.Context, exchange *oauth2.Token, _ url.Values) (*Claims, error) {
	...
	if err = json.NewDecoder(resp.Body).Decode(&user); err != nil {
		return nil, errors.WithStack(herodot.ErrUpstreamError.WithWrap(err).WithReasonf("reason %s", err))
	}
	...
	claims := &Claims{
		Issuer:      "https://oauth-ift.sber.ru/ru/prod/sberbankid/v2.1/userinfo",
		Subject:     user.Sub,
...
func (g *ProviderSberIft) Exchange(ctx context.Context, code string, opts ...oauth2.AuthCodeOption) (*oauth2.Token, error) {
	ctx, cancel := context.WithTimeout(ctx, sberAPITimeout)
	defer cancel()
	...
	if err != nil {
		...
		if statusCode, bodyFragment, ok := extractOAuth2RetrieveError(err); ok {
			...
			reason := "sber token exchange failed"
			if statusCode == http.StatusUnauthorized {
				reason = "sber token exchange unauthorized"
			}
			...
			return nil, errors.WithStack(
				herodot.ErrUpstreamError.
					WithWrap(err).
					WithReasonf("%s: debug_version=%s stage=token_exchange provider=%s token_endpoint=%s request_id=%s http_status=%d response=%q tls_client_cert_present=%t tls_client_config_absent=%t mtls_cert_path=%q mtls_key_path=%q mtls_attach_error=%q curl_request=%q",
						reason,
						sberTokenDebugVersion,
						g.config.ID,
						tokenEndpoint,
