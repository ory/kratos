...
func enablePasskeys(conf *config.Config) {
	ctx := context.Background()
	passkey := config.ViperKeySelfServiceStrategyConfig + "." + string(identity.CredentialsTypePasskey)
	conf.MustSet(ctx, passkey+".enabled", true)
	conf.MustSet(ctx, passkey+".config.rp.display_name", "Ory Corp")
	conf.MustSet(ctx, passkey+".config.rp.id", "localhost")
	conf.MustSet(ctx, passkey+".config.rp.origins", []string{"http://localhost:4455"})
}
...
func TestRegistration(t *testing.T) {
	...
	t.Run("successful registration", func(t *testing.T) {
		t.Cleanup(func() {
			conf.MustSet(
				ctx,
				config.HookStrategyKey(config.ViperKeySelfServiceRegistrationAfter, identity.CredentialsTypePasskey.String()),
				nil,
			)
		})
