...
func (s *Strategy) hydratePassKeyRegistrationOptions(ctx context.Context, f *registration.Flow) error {
	defaultSchemaURL, err := s.d.Config().DefaultIdentityTraitsSchemaURL(ctx)
	if err != nil {
		return err
	}

	if gjson.GetBytes(f.InternalContext, flow.PrefixInternalContextKey(s.ID(), InternalContextKeySessionData)).IsObject() {
		// Node already exists, skip re-creation.
		return nil
	}
