...
func (s *Sender) SendCode(ctx context.Context, f flow.Flow, id *identity.Identity, addresses ...Address) error {
	...
	for _, address := range addresses {
		...
		var rawCode string
		if s.deps.Config().SelfServiceCodeStrategy(ctx).ExternalSMSVerify.Enabled && address.Via == identity.AddressTypeSMS {
			rawCode = "external"
		} else {
...
func (s *Sender) SendVerificationCode(ctx context.Context, f *verification.Flow, via string, to string) error {
	...
	var rawCode string
	if s.deps.Config().SelfServiceCodeStrategy(ctx).ExternalSMSVerify.Enabled && via == identity.AddressTypeSMS {
		rawCode = "external"
	} else {
		rawCode = GenerateCode()
	}
