...
context('MFA Profile', () => {
  describe('Test TOTP', () => {
    ...
    it('should go through several totp lifecycles', () => {
      ...
      cy.get('p[data-testid="text-totp_secret_key-content"]').should('not.exist')
      ...
      cy.get('form .messages .message').should('contain.text', 'The provided authentication code is invalid, please try again.')
      cy.get('input[name="totp_code"]').then(($e) => {
        cy.wrap($e).type(authenticator.generate(secret))
      })
      cy.get('*[name="method"][value="totp"]').click()
