...
module.exports = {
  APP_URL: (Cypress.env('app_url') || 'http://localhost:4455').replace(
    /\/$/,
    ''
  ),
  MOBILE_URL: (Cypress.env('mobile_url') || 'http://localhost:4457').replace(
    /\/$/,
    ''
  ),
  KRATOS_ADMIN: (Cypress.env('kratos_admin') || 'http://localhost:4434')
    .replace()
    .replace(/\/$/, ''),
  KRATOS_PUBLIC: (Cypress.env('kratos_public') || 'http://localhost:4433')
    .replace()
    .replace(/\/$/, ''),
  MAIL_API: (Cypress.env('mail_url') || 'http://localhost:4437').replace(
    /\/$/,
    ''
  ),
