...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		r, err := driver.NewRegistry(c)
		if err != nil {
			logger.WithError(err).Fatal("Unable to initialize driver")
		}
...
func init() {
	...
	// serveAllCmd.PersistentFlags().String("foo", "", "A help for foo")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// serveAllCmd.Flags().BoolP("toggle", "t", false, "Help message for toggle")
}
