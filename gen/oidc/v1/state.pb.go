...
type State struct {
	...
	// request_base_url is the scheme://host[:port] the browser used at flow
	// init. It is captured so the OIDC/SAML callback can be redirected back
	// onto the developer proxy / custom domain the user started on. Empty
	// when there is nothing to rewrite.
	RequestBaseUrl string `protobuf:"bytes,6,opt,name=request_base_url,json=requestBaseUrl,proto3" json:"request_base_url,omitempty"`
...
func (x *State) GetRequestBaseUrl() string {
	if x != nil {
		return x.RequestBaseUrl
	}
	return ""
}
...
const file_oidc_v1_state_proto_rawDesc = "" +
	"\n" +
	"\x13oidc/v1/state.proto\x12\aoidc.v1\"\x8c\x02\n" +
	"\x05State\x12\x17\n" +
	...
	"\tflow_kind\x18\x05 \x01(\x0e2\x11.oidc.v1.FlowKindR\bflowKind\x12(\n" +
	"\x10request_base_url\x18\x06 \x01(\tR\x0erequestBaseUrl*n\n" +
	"\bFlowKind\x12\x19\n" +
