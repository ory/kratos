...
func NewKratosServerWithCSRF(t *testing.T, reg driver.Registry) (public, admin *httptest.Server) {
	...
	if len(reg.Config().Source().String(config.ViperKeySelfServiceLoginUI)) == 0 {
		reg.Config().MustSet(config.ViperKeySelfServiceLoginUI, "http://NewKratosServerWithCSRF/you-forgot-to-set-me/login")
	}
	reg.Config().MustSet(config.ViperKeyPublicBaseURL, public.URL)
	reg.Config().MustSet(config.ViperKeyAdminBaseURL, admin.URL)
...
func InitKratosServers(t *testing.T, reg driver.Registry, public, admin *httptest.Server) {
	if len(reg.Config().Source().String(config.ViperKeySelfServiceLoginUI)) == 0 {
		reg.Config().MustSet(config.ViperKeySelfServiceLoginUI, "http://NewKratosServerWithRouters/you-forgot-to-set-me/login")
	}
	reg.Config().MustSet(config.ViperKeyPublicBaseURL, public.URL)
	reg.Config().MustSet(config.ViperKeyAdminBaseURL, admin.URL)
