...
func (m *RegistryDefault) FailedLoginHooks(ctx context.Context) (b []login.FailedHookExecutor) {
	for _, v := range m.getHooks("", m.Config().SelfServiceFlowLoginFailedHooks(ctx)) {
		if hook, ok := v.(login.FailedHookExecutor); ok {
			b = append(b, hook)
		}
	}
	return
