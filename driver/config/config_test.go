	...
	t.Run("suite=loaders", func(t *testing.T) {
		...
		t.Run("group=methods", func(t *testing.T) {
			for _, tc := range []struct {
				...
				{id: "password", enabled: true, config: `{"haveibeenpwned_host":"api.pwnedpasswords.com","haveibeenpwned_enabled":true,"ignore_network_errors":true,"max_breaches":0,"min_password_length":6,"identifier_similarity_check_enabled":true}`},
