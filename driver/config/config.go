...
func (p *Config) SelfServiceFlowRegistrationTwoSteps(ctx context.Context) bool {
	...
	case "profile_first":
		if p.GetProvider(ctx).BoolF(ViperKeySelfServiceRegistrationEnableLegacyOneStep, false) {
			p.l.Warnf("Configuration key %q is deprecated and is conflicting with %q. Ignoring value from %q and using %q.", ViperKeySelfServiceRegistrationEnableLegacyOneStep, ViperKeySelfServiceRegistrationFlowStyle, ViperKeySelfServiceRegistrationEnableLegacyOneStep, style)
		}
		return true
	default:
		if !p.GetProvider(ctx).BoolF(ViperKeySelfServiceRegistrationEnableLegacyOneStep, false) {
