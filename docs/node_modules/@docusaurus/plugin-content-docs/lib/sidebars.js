...
function normalizeItem(item) {
    ...
    if (isCategoryShorthand(item)) {
        return lodash_1.default.flatMap(normalizeCategoryShorthand(item), normalizeItem);
    }
    switch (item.type) {
        case 'category':
            assertIsCategory(item);
            return [Object.assign(Object.assign({}, item), { items: lodash_1.default.flatMap(item.items, normalizeItem) })];
        case 'link':
            assertIsLink(item);
            return [item];
        case 'ref':
...
function normalizeSidebar(sidebars) {
    return Object.entries(sidebars).reduce((acc, [sidebarId, sidebar]) => {
        ...
        acc[sidebarId] = lodash_1.default.flatMap(normalizedSidebar, normalizeItem);
        return acc;
    }, {});
}
