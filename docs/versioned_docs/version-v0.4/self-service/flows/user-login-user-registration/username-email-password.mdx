...
ORY Kratos uses the Identity JSON Schema defined in `identity.default_schema_url`
to generate a list of form fields and add it to the Registration Request. Using
an Identity JSON Schema like
...
Public API. The data will be validated against the Identity JSON Schema (e.g. checking if
...
Registration sequence, this payload does not change when the Identity
JSON Schema changes:
