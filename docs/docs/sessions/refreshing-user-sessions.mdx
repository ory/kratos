...
Since session refresh causes db writes it's possible to set a config value to allow session refresh only for a specific
time before the session expires. If we set the `refresh_min_time_left` equal to `lifespan`,
it's possible to refresh even new sessions.
...
By default refresh isn't possible on this endpoint and needs to be enabled through the config.
...
If we don't want to use the public whoami endpoint for session refresh, we can do it via the admin endpoints.
