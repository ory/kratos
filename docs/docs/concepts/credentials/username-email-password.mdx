...
Ory Kratos can hash passwords by BCrypt and can compare stored BCrypt hash and migrate if configured hasher (`hashers.algorithm`) is not BCrypt.
...
Ory Kratos can hash passwords by Argon2 and can compare stored Argon2 hash and migrate if configured hasher (`hashers.algorithm`) is not Argon2.
...
But Kratos doesn't have ability to import credentials now. There is an open issue for this feature:[Issue](https://github.com/ory/kratos/issues/605).
