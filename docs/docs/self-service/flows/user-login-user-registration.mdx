...
Info"_ depend, of course, on the actual Login / Registration Method being used.
But it is important to understand that **"Your Application"** is responsible for
...
1. ORY Kratos does some internal processing (e.g. checks if a session cookie is
   ...
   redirected to `https://127.0.0.1:4455/auth/(login|registration)?flow=abcde`.
...
1. The User's browser makes a request to one of ORY Kratos' Strategy URLs (e.g.
   ...
   - If the flow is a registration flow and the registration data is valid, the
     identity is created.
...
`https://127.0.0.1:4455/.ory/kratos/public/self-service/login/browser` MUST be
