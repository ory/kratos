...
1. ORY Kratos does some internal processing (e.g. checks if a session cookie is
   ...
   `selfservice.flows.login.ui_url` (or `selfservice.flows.registration.ui_url`) config or `SELFSERVICE_FLOWS_LOGIN_UI_URL` (or
   `SELFSERVICE_FLOWS_REGISTRATION_UI_URL`) environment variable, which is set to the ui
