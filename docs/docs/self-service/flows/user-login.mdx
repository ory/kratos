...
- `oidc` for signing in using a social sign in provider such as Google or Facebook (check out the [set up guide](../../guides/sign-in-with-github-google-facebook-linkedin.mdx)).
...
To initialize the API flow, the client calls the API-flow initialization endpoint ([REST API Reference](../../reference/api.mdx#initialize-login-flow-for-api-clients))
...
Fetching the Login Flow ([REST API Reference](../../reference/api.mdx#get-login-flow)) is usually only required for browser clients but also works
