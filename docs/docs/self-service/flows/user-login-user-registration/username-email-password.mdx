...
and
[Self-Service Flows Documentation](../../../self-service)
first.
...
Browser clients are server-side applications that run in a browser, or Single Page Apps (ReactJS, AngularJS, ...).
...
API clients are applications which do not run in a browser or in a URL (e.g. `https://my-example.org`). These
are typically native apps (e.g. iOS, Android).

### Registration

The registration is initiated by making a request to the Registration API Flow Initialization endpoint:
...
As you can see, the flow content is very similar to the one for browser flows. The only difference is that
`type` is now `api` and not `browser`.

This payload then renders that payload as a native form. The implementation depends on the language, platform, and
framework you use.

To submit the form, the client makes a request to the action URL which results in an error here because the
...
If the `session` hook is configured, you will also receive a session and a session token:
...
The login is initiated by making a request to the Login API Flow Initialization endpoint:
...
If user and password are correct, the server responds with a session token and a session:
