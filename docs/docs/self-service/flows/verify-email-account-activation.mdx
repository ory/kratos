...
To initialize the API flow, the client calls the API-flow initialization endpoint ([REST API Reference](../../reference/api.mdx#initialize-verification-flow-for-api-clients))
...
The `link` verification mode will always open a link in the browser, even if it was initiated by an API client.
