...
1. The User's browser makes a request to one of ORY Kratos' Strategy URLs (e.g.
   ...
   - If the form data is invalid, the Settings Request's JSON Payload is updated -
     for example with
     ```json5
     {
       id: 'abcde',
   ...
   - If the data is valid but the modifications require proof of identity (e.g.
     ...
     flow is executed. Only after a successful settings flow will the changed data
     be stored!
