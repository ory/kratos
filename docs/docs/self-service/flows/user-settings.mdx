...
its [Identity Traits](../../concepts/identity-user-model).
...
- The `oidc` method allows linking and unlinking from Google, Facebook, Github, ... which is considered a privileged action.
...
a 403 Forbidden status message which require you to request a new ORY Kratos Login session using the [API-based Flow](user-login).
...
Each Settings Flow also has a `state` field which can either be `success` or `show_form`:
...
To initialize the API flow ([REST API Reference](../reference/api#getSelfServiceSettingsFlow)), the client [calls the API-flow initialization endpoint](../../self-service#initialization)
