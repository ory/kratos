...
Please read through the [User Settings Documentation](../user-settings.mdx) first.
...
[Identity Traits](../../../concepts/identity-user-model). If one or more
...
initiates the flow. The Settings Request Response Payload always include a
`profile` method:
