...
ORY Kratos allows users to change their settings (e.g. update password, change
address, ...) using two principal flows:
...
This document explains the high-level flow for managing settings
and documents the primary profile strategy ("update your profile information").

Changing settings for *"Username or Email
and Password"* ("change your password") or *"Social Sign In"* ("un-/link a
social profile") strategies [is documented in Self-Service Strategies](../strategies/index.md).
...
```json title="path/to/identity.traits.schema.json"
{
