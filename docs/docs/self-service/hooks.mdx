...
- `instance_ptr` points to the field in the payload/schema that this error
  refers to and will be used by the frontend to display an error next to the
  correct form field (check documentation for the given self-service flow)
- `id` is the numeric error ID that can be used to translate messages in the
  front end.
