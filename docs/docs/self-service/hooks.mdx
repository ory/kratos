...
flow into the jsonnet template. If available, `session` and `identity` objects
are bound into the jsonnet template as well. These objects are available through
a `ctx` object. E.g. to create a body looking like `{ user_id: <some-id> }` to
