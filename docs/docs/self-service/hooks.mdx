...
"hook 1" is only run for "strategy 2", but not for "strategy 1" as latter defines its own hooks.

## All flows
Following hooks can be configured for all flows.

### Web-Hooks
Web-Hooks can be configured for every, but error and logout flow.
...
the flow if the call to the Web-Hook end point fails.

:::

The configuration for a web-hook goes into the `hooks` property either on the flow, or the method
